...
describe('PaymentService', () => {
  ...
  const resetToPrePaymentState = async () => {
    await db
      ...
      .where(eq(bookings.id, testBookingIds[0]));
    ...
    await db.delete(payments).where(eq(payments.bookingId, testBookingIds[0]));
  };
  ...
  afterAll(async () => {
    await db.transaction(async (tx) => {
      await tx
        .delete(payments)
        .where(inArray(payments.bookingId, testBookingIds));
      await tx.delete(bookings).where(inArray(bookings.id, testBookingIds));
  ...
  describe('createPaymentOrder', () => {
    ...
    it('should create a razorpay order and return order details', async () => {
      ...
      const [paymentRecord] = await db
        .select()
        .from(payments)
        .where(eq(payments.bookingId, testBookingIds[0]));
    ...
    it('should throw ConflictError if booking is not pending', async () => {
      ...
      await expect(
        PaymentService.createPaymentOrder(testBookingIds, testUserId)
  ...
  describe('handlePaymentSuccess', () => {
    ...
    it('should verify signature, mark booking completed, seat sold', async () => {
      ...
      const updatedBookings = await PaymentService.handleSuccess({
      ...
      const [paymentRecord] = await db
        ...
        .where(
          and(
            eq(payments.bookingId, testBookingIds[0]),
            eq(payments.event, 'payment_captured')
          )
        );
    ...
    it('should record payment_failed, mark booking failed, release seat', async () => {
      ...
      const [paymentRecord] = await db
        ...
        .where(
          and(
            eq(payments.bookingId, testBookingIds[0]),
            eq(payments.event, 'payment_failed')
          )
        );
      expect(paymentRecord).toBeDefined();
      expect(paymentRecord.gatewayOrderId).toBe(gatewayOrderId);
      expect(paymentRecord.gatewayResponse).toBe(
    ...
    it('should initiate full refund and release seat', async () => {
      ...
      const [paymentRecord] = await db
        ...
        .where(
          and(
            eq(payments.bookingId, testBookingIds[0]),
            eq(payments.event, 'refund_initiated')
          )
        );
      expect(paymentRecord).toBeDefined();
  ...
  describe('handleWebhook', () => {
    ...
    it('should record refund_completed when webhook is valid', async () => {
      ...
      const [paymentRecord] = await db
        ...
        .where(
          and(
            eq(payments.bookingId, testBookingIds[0]),
            eq(payments.event, 'refund_completed')
          )
        );
      expect(paymentRecord).toBeDefined();
    ...
    it('should throw UnauthorizedError if signature verification fails', async () => {
      ...
      await expect(
        PaymentService.handleWebhook({ rawBody, signature: 'bad_signature' })
      ).rejects.toThrow(UnauthorizedError);
    });
