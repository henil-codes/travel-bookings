...
export class PaymentService {
    ...
    static async handleSuccess(input: {
        ...
        if (expectedSignature !== input.gatewayPaymentSignature) {
        ...
        return await db.transaction(async (tx) => {
            ...
            if (booking.status !== 'pending') {
                throw new ConflictError('Cannot capture payment for booking with status ' + booking.status)
            }
            ...
            await tx.update(seats).set({ status: 'sold' }).where(eq(seats.id, booking.seatId))
    ...
    static async handleFailure(input: {
        ...
        return await db.transaction(async (tx) => {
            ...
            if (booking.status !== 'pending') {
                throw new ConflictError('Cannot record payment failure for booking with status ' + booking.status);
            }
            ...
            await tx.update(seats).set({ status: 'available' }).where(eq(seats.id, booking.seatId))
    ...
    static async initiateRefund(input: {
        ...
        return await db.transaction(async (tx) => {
            ...
            const [capturedPayment] = await tx.select().from(payments).where(eq(payments.bookingId, input.bookingId)).orderBy(payments.createdAt).limit(1);
            ...
            // release seat
            await tx.update(seats).set({ status: 'available', lockedUntil: null, lockedByUserId: null }).where(eq(seats.id, booking.seatId))
    ...
    static async handleWebhook(input: {
        rawBody: string;
        signature: string;
    }) {
        const expectedSignature = crypto.createHmac('sha256', process.env.RAZORPAY_WEBHOOK_SECRET!).update(input.rawBody).digest('hex')
        ...
        if (event.event === 'refund.processed') {
            ...
            const [existingRefund] = await db.select().from(payments).where(eq(payments.gatewayRefundId, refundId));
