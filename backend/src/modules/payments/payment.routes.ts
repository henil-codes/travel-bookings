...
export const paymentRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
    ...
    server.post('/:bookingId/order', {
        ...
        const order = await PaymentService.createPaymentOrder(request.params.bookingId);
    ...
    server.post('/verify', {
        preHandler: [fastify.authenticate],
        schema: {
            description: 'Verifies Razorpay payment signature and confirms booking.',
            tags: ['Payments'],
