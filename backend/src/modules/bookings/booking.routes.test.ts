...
describe('Booking Routes Integration', () => {
    ...
    beforeAll(async () => {
        ...
        const [vehicle] = await db.insert(vehicles).values({
            operatorName: 'Booking Route Operator', vehicleNumber: 'BR-001',
    ...
    afterAll(async () => {
        await db.transaction(async (tx) => {
            ...
            await tx.delete(users).where(eq(users.email, 'booking.customer@test.com'));
    ...
    const post = (path: string, body: object, token?: string) => fetch(`${serverUrl}/api/v1${path}`, {
        method: 'POST',
        headers: {
    ...
    const get = (path: string, token?: string) => fetch(`${serverUrl}/api/v1${path}`, {
    ...
    describe('GET /bookings/:id', () => {
        ...
        it('should return 404 for not-existing booking', async () => {
            const res = await get(`/bookings/00000000-0000-0000-0000-000000000000`, adminToken);
