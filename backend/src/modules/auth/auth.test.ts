...
describe('AuthService', () => {
  ...
  describe('AuthService - googleOAuth()', () => {
    ...
    afterAll(async () => {
      await db.delete(users).where(eq(users.email, googleUser.email));
      await db.delete(users).where(eq(users.email, 'conflict@example.com'));
    });

    it('should create a new user on first google login', async () => {
      const result = await AuthService.googleOAuth(googleUser);
    ...
    it('should store googleId in the database', async () => {
      const [userRecord] = await db
        .select()
        .from(users)
      ...
      expect(userRecord.googleId).toBe(googleUser.googleId);
    });

    it('should leave countryCode and localPhone null for google users', async () => {
      const [userRecord] = await db
        .select()
        .from(users)
        .where(eq(users.email, googleUser.email));
      ...
      expect(userRecord).toBeDefined();
      expect(userRecord.countryCode).toBeNull();
      expect(userRecord.localPhone).toBeNull();
    });

    it('should return existing user if googleId already exists', async () => {
      const first = await AuthService.googleOAuth(googleUser);
      const second = await AuthService.googleOAuth(googleUser);
    ...
    it('should throw ConfictError if email already exists with a different provider', async () => {
      const passwordHash = await bcrypt.hash('somepassword', 10);
      await db.insert(users).values({
        name: 'Conflict User',
        email: 'conflict@example.com',
        countryCode: '+1',
        localPhone: '6666666666',
        passwordHash,
        authProvider: 'local',
        accountStatus: 'active',
      });
      ...
      await expect(
        AuthService.googleOAuth({
          ...
          email: 'conflict@example.com',
          name: 'Google Conflict User',
        })
      ).rejects.toThrow(ConflictError);
    });

    it('should include "local" in the conflict message', async () => {
      await expect(
        AuthService.googleOAuth({
          googleId: 'google-sub-conflict-2',
          email: 'conflict@example.com',
          name: 'Google Conflict User 2',
        })
      ).rejects.toThrow(/local/);
    });
  });

  describe('AuthService - google OAuth callback', () => {
    let app: FastifyInstance;
    let serverUrl: string;
