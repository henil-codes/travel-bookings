...
describe('Trip Routes Integration', () => {
  ...
  beforeAll(async () => {
    ...
    const [admin] = await db
      ...
      .values({
        name: 'Admin User',
        email: 'triproute.admin@test.com',
        countryCode: '+91',
    ...
    const [customer] = await db
      ...
      .values({
        name: 'Customer User',
        email: 'triproute.customer@test.com',
        countryCode: '+91',
    ...
    const trip = await TripService.createTrip({
        ...
        arrivalTime: new Date(Date.now() + 936000000).toISOString(),
  ...
  describe('GET /trips/:id/seats', () => {
    ...
    it('should return correct summary counts', async () => {
      const res = await get(`/trips/${testTripId}/seats`);
      const body = await res.json();
      expect(body.data.summary.total).toBe(40);
      expect(body.data.summary.available).toBe(40);
      expect(body.data.summary.locked).toBe(0);
      expect(body.data.summary.sold).toBe(0);
  ...
  describe('POST /trips', () => {
    ...
    afterAll(async () => {
      if (createdTripId) {
        await db.delete(seats).where(eq(seats.tripId, createdTripId));
  ...
  describe('DELETE /trips/:id', () => {
    ...
    afterAll(async () => {
      if (deletableTripId) {
          await db.delete(seats).where(eq(seats.tripId, deletableTripId));
